...
// Class declaration
[System.Serializable]
public class MyEvent3 : UnityEvent<int> { }

public class TCPServer3 : MonoBehaviour
{
    private MyEvent3 OnDevicesArrived;

    public GameObject prefabPointCloud;

    private StateObject3 state;
        ...
        lock (pointcloud_lock)
        {
            if (devices_parameters_arrived && !pointcloud_instantiated)
            {
                ...
                for (int i = 0; i < n_devices; i++)
                {
                    ...
                    displayPointCloud.Frame = new byte[displayPointCloud.FrameRawDimension /*+ StateObject1.BufferSize*/];
                    ...
                    if (!FRAME_CREATED)
                    {
                        FRAME = new byte[width * heigth * (3 + 2) + StateObject3.BufferSize];
                        FRAME_CREATED = true;
                    }
    ...
    public void AcceptCallback(IAsyncResult ar)
    {
        ...
        state = new StateObject3();
        state.workSocket = handler;
        handler.BeginReceive(state.buffer, 0, StateObject3.BufferSize, 0,
    ...
    private void ReadCallback(IAsyncResult ar)
    {
        ...
        StateObject3 state = (StateObject3)ar.AsyncState;
