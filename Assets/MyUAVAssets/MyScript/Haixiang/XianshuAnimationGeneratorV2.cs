...
public class XianshuAnimationGeneratorV2 : AnimationGenerator
{
    public GameObject ropePrefab;
    public GameObject forwardKagu;
    public GameObject backwardKagu;
    public bool isMain = false;
    public float lengthOffset = 0;
    public Transform startPos;
    public List<Transform> ForwardPathList;
    public List<Transform> BackwardPathList;
    ...
    private void AddMesh(GameObject obj, GameObject kagu)
    {
        //MeshFilter meshFilter = obj.AddComponent<MeshFilter>();
        //meshFilter.mesh = kagu.GetComponent<MeshFilter>().mesh;
        //MeshRenderer mr = obj.AddComponent<MeshRenderer>();
        MeshFilter kaguFilter = kagu.GetComponent<MeshFilter>();
        UnityEditorInternal.ComponentUtility.CopyComponent(kaguFilter);
        UnityEditorInternal.ComponentUtility.PasteComponentAsNew(obj);

        MeshRenderer kaguMr = kagu.GetComponent<MeshRenderer>();
        UnityEditorInternal.ComponentUtility.CopyComponent(kaguMr);
        UnityEditorInternal.ComponentUtility.PasteComponentAsNew(obj);
    }
    ...
    private void RopeInit()
    {
        ...
        GameObject forwardControlPoint = ropeObj.transform.Find("ForwardControlPoint").gameObject;
        GameObject backwardControlPoint = ropeObj.transform.Find("BackwardControlPoint").gameObject;
        if (forwardKagu != null)
        {
            AddMesh(forwardControlPoint, forwardKagu);
        }
        if (backwardKagu != null)
        {
            AddMesh(backwardControlPoint, backwardKagu);
        }
    }
    ...
    private void SetRopeLength()
    {
        ...
        for (int i = 1; i < ForwardPathList.Count; i++)
        {
            leftLength += (ForwardPathList[i].transform.position - ForwardPathList[i - 1].transform.position).magnitude;
        }
        for (int i = 1; i < BackwardPathList.Count; i++)
        {
            rightLength += (BackwardPathList[i].transform.position - BackwardPathList[i - 1].transform.position).magnitude;
        }
    ...
    public void GenerateAnimation()
    {
        ...
        if (ForwardPathList != null)
            GenerateControlPointAnimation(forwardControlPoint, ForwardPathList);
        if (BackwardPathList != null)
            GenerateControlPointAnimation(backwardControlPoint, BackwardPathList);
